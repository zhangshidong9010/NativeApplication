            ...
            v?.let {
                when (it.id) {
                    ...
                    R.id.string_length -> {
                        ...
                        Toast.makeText(this, "字符串\"hello zhangshidong\"的length：$length", Toast.LENGTH_SHORT).show()
                    }
                    R.id.sub_string -> {
                        var subString = Native.subString("I am zhangshidong", 3)
                        Toast.makeText(this, "字符串I am zhangshidong截取后为：$subString", Toast.LENGTH_SHORT).show()
                    }
                    R.id.char_index -> {
                        var char = Native.getCharWithIndex("hello zhangshidong", 6).toString()
                        Toast.makeText(this, "字符串hello zhangshidong中下标为6的字符为：$char", Toast.LENGTH_SHORT).show()
                    }
                    ...
                    R.id.array_length -> {
                        ...
                        Toast.makeText(this, "数组(1,2,3,4,5,6)的length：$length", Toast.LENGTH_SHORT).show()
                    }
                    R.id.element_index -> {
                        var array = arrayOf("1", "2", "3", "4", "5")
                        var str = Native.getElementFromArray(3, array)
                        Toast.makeText(this, "数组(\"1\",\"2\",\"3\",\"4\",\"5\")中下标为3的元素为$str", Toast.LENGTH_SHORT).show()
                    }
                    R.id.set_element_index -> {
                        var array = arrayOf("1", "2", "3", "4", "5")
                        Native.setObjectArrayElement(array, 2, "10")
                        Toast.makeText(this, "(\"1\",\"2\",\"3\",\"4\",\"5\") => ${array.toList()}", Toast.LENGTH_SHORT).show()
                    }
                    R.id.sub_array -> {
                        var array = intArrayOf(1, 2, 3, 4, 5, 6)
                        var newArray = Native.subArray(array, 2, 4)
                        Toast.makeText(this, "截取前array:${array.toList()}, 截取后newArray:${newArray.toList()}", Toast.LENGTH_SHORT).show()
                    }
                    R.id.sort_array -> {
                        var array = intArrayOf(10, 2, 3, 140, 51, 6, 0, 11)
                        var srcArray = array.toList().toString()
                        Native.sortArray(array)
                        Toast.makeText(this, "array排序前$srcArray, array排序后${array.toList()}", Toast.LENGTH_SHORT).show()
                    }
                    ...
                    R.id.native_set_java_custom_property -> {
                        ...
                        Toast.makeText(this, "customName初始值：$customName, 改变后的值：${nativeCallJava.customName}", Toast.LENGTH_SHORT).show()
                    }
                    R.id.native_set_java_static_property -> {
                        ...
                        Toast.makeText(this, "staticName初始值：$staticName, 改变后的值：${NativeCallJava.staticName}", Toast.LENGTH_SHORT).show()
                    }
                    R.id.native_call_java_custom_method -> {
                        var str = Native.nativeCallJavaCustomMethod()
                        Toast.makeText(this, "NativeCallJavaCustomMethod: $str", Toast.LENGTH_SHORT).show()
                    }
                    R.id.native_call_java_static_method -> {
                        var str = Native.nativeCallJavaStaticMethod()
                        Toast.makeText(this, "NativeCallJavaStaticMethod: $str", Toast.LENGTH_SHORT).show()
                    }
                    ...
                    R.id.native_create_java_object -> {
                        var objects = Native.nativeCreateJavaObject(NativeCallJava::class.java) as NativeCallJava
                        var result: String = if (objects != null) "对象创建成功${objects.toString()}" else "对象创建失败null"
                    ...
                    R.id.java_reference_native_object -> {
                        val mJniPerson = JniPerson()
                        mJniPerson.setAge(68)
                        mJniPerson.setName("zhangshidong")
                        val name = mJniPerson.getName()
                        val age = mJniPerson.getAge()
                        Toast.makeText(this, "My name is $name,  I am $age years old", Toast.LENGTH_SHORT).show()
                    }
